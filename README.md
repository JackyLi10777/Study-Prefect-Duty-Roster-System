...
>
> **為何服事 · 非以役人，乃役於人。** — 取意自馬可福音 10:45 的服務原則，並非逐字經文
>
> **如何服事 ·「因此，我勉勵自己，對　神對人，時常存着無虧的良心。」** — [使徒行傳 24:16，和合本修訂版 2010（神版）](https://www.bible.com/bible/2625/ACT.24.16.%E5%92%8C%E5%90%88%E6%9C%AC2010%20-%20%E7%A5%9E%E7%89%88)
>
> 無虧的良心是復活盼望與福音恩典所結出的忠心操練，不是靠行為換取救恩，也不是宣稱自己毫無罪過。
...
> **手機優先操作候選（尚未部署）：** `codex/mobile-first-operations` 正把 Android／iPhone 的每週主流程收斂為「選擇週次 → 停開／請假 → 規則摘要 → 準備狀態 → 生成」。手機抽屜使用四個獨立圓角狀態方格，不再繼承桌面圓形旋鈕，因此提示音及外觀按鈕不會出現橢圓光圈或逐字直排；抽屜打開時底部導航會同時視覺隱藏及語意停用。草稿在手機以單日六崗位、星期摘要、共用底部編輯面板及單一未保存操作列呈現；桌面仍保留 PDF 式矩陣，交易、CAS、command ID、公平及 PDF 語意沒有改動。真實 Chrome 的隔離裝置矩陣已通過，並以量度結果停用造成約 1.5 秒啟動長任務的 Tailwind 瀏覽器編譯器，改用 NiceGUI 內置 UnoCSS `wind3`；reduced-motion 路徑亦不再下載 GSAP。完整候選驗收見[手機操作驗收](docs/MOBILE_OPERATIONS_ACCEPTANCE.md)。這段只描述未部署來源；完整 release gate、正式部署及實體 Android 驗收仍未完成，正式狀態以上方生成區塊為準。
...
工作台的正式寫入、報告、匯出、備份及還原使用共用而誠實的等待狀態。在 140ms 內完成的操作只保留按壓／圖標回饋，不閃現載入視窗；仍在處理但無法量度的操作只顯示 indeterminate 狀態。只有後端提供真實、單調遞增的階段事件或 `completed／total` 時，介面才顯示階段名稱或數值，不會以固定「準備 → 安全處理 → 完成」、14% 或 56% 假裝進度。結果一旦可用便立即呈現，系統不會為了播放動畫而加入最低等待時間。跨頁跳轉另以 150ms 為頂部細軌門檻，完成、返回或離頁即清理。若正式寫入逾時，不代表操作失敗，系統不會自動重送；先依畫面狀態及 OP／REQ 編號核對。Guest 禁止的匯入、上載、正式保存或高成本功能會在 loading 前被權限層拒絕。所有狀態均保留鍵盤、forced-colours 及 reduced-motion 的靜態等價呈現。
...
rc53 來源候選的草稿與正式輸出會明確區分五種狀態：`assigned`（已安排）、`vacant`（空缺／待安排）、`room_closed`（按固定政策不開放）、`unavailable`（本週指定單格不開放）及 `day_closed`（本週指定日期全天不開放）。狀態優先序為全天不開放 → 固定房間不開放 → 單格不開放 → 已安排／空缺。空白搜尋框不會自動刪除安排或建立空缺；輸入 `X`、`×`、「空缺」或「待安排」後，仍須選擇明確的「設為空缺」。單格或整日重新開放後只顯示空缺，不會靜默恢復舊人選。已發布週表不可直接改停開狀態，必須先安全撤回；Guest 可完整演練相同行為，但只修改記憶體內的虛構工作區。完整鍵盤、觸控、拖放、衝突復原及交接說明見[草稿試算表與全天停開操作](docs/ROSTER_DRAFT_EDITING.md)。這段在 rc53 正式發布前只描述來源候選；目前線上狀態仍以[目前系統狀態](docs/status/CURRENT_STATUS.md)為準。
...
我與 Codex 採用風險導向審查，而不是以「逐行看過」代替可重複證據。每次正式候選會核對政策與交易不變量、異常及復原路徑、硬編碼配置、供應鏈、瀏覽器生命週期、備份／隔離還原及真實 Cloudflare 交付。依賴安全修復必須同步更新正式 runtime、開發鎖檔及 Worker 鎖檔，分別通過 Python 與 Node 供應鏈掃描；只改直接版本或只通過功能測試均不可宣稱已完成。名單檔案限定 2 MB 且只接受 CSV／普通 XLSX；系統在解析前拒絕巨集、公式、異常壓縮比例、過大解壓內容及無法辨識的編碼。外部 AI／Viewer 請求有 HTTPS 目的地限制、逾時、回應大小上限、可重試或冪等語意及雙語失敗提示。
...
README、架構文件及發布報告中的工程成果亦整理成獨立網站介面。它以完整自動化測試套件、目前發布報告的實際閘門比例、五層系統藍圖、可靠性工程能力及建造脈絡說明品質；目前候選驗證器有 15 道閘門，包括 Cloudflare Worker 的 Deno 契約、獨立圖標狀態機、16 組主題交接、桌面瀏覽器、手機適應、效能、記憶體穩定性、統一 Guest 隔離及完整寫入／復原。展示數字只來自仍與目前可部署原始碼指紋及驗證後 clean-tree binding 相符的報告，不會加入使用人數、商業成效或其他虛假 KPI。
...
修改 Admin 讀寫、Guest 隔離／capacity、Worker service binding／WebSocket、下載、備份、outbox concurrency 或 Viewer 發布／解密時，另執行 `python -X utf8 scripts\verify_mixed_gateway_load.py`。它以實際 Worker source、local workerd 及虛構 disposable SQLite 提供有界混合負載證據；[日期化驗收](docs/audits/MIXED_GATEWAY_LOAD_ACCEPTANCE_2026-08-01.md)明確區分本機基線、正式 edge、部署及真人驗收。
