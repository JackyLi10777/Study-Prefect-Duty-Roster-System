...
草稿與正式輸出會明確區分四種狀態：`assigned`（已安排）、`vacant`（空缺／待安排）、`room_closed`（按固定政策不開放）及 `day_closed`（本週指定日期全天不開放）。空白搜尋框不會自動刪除安排或建立空缺；輸入 `X`、`×`、「空缺」或「待安排」後，仍須選擇明確的「設為空缺」。關閉已有安排的日期會在保存前列明將移除的安排；重新開放後只顯示空缺，不會靜默恢復舊人選。已發布週表不可直接改停開日，必須先安全撤回；Guest 可完整演練相同行為，但只修改記憶體內的虛構工作區。完整鍵盤、手機、衝突復原及交接說明見[草稿試算表與全天停開操作](docs/ROSTER_DRAFT_EDITING.md)。
