...
本專案並非簡單的個人工具——它的開發過程本身就是一次大規模的 **AI 輔助軟件工程實踐**。
...
這些 AI 資源的投入直接反映在專案的每一個層面：

- **5 輪架構重構**：從最初的單文件腳本，逐步演進為 7 層模組化架構。School Policy SSOT、Session State 集中管理、結構化異常層次——每一輪重構均由 AI 輔助分析與實施。
...
- **雙語文檔體系**：中文與英文 README 達到完全結構對等，使用說明書支援中英雙語——AI 輔助翻譯與潤色確保專業品質。
...
> 約 20 億 tokens 的 AI 投入，最終凝結為一個 **部署在 Streamlit Cloud 上、62 項測試護航、10 張架構圖詳解、雙語完整文檔** 的專業級系統。


### 領導與決策

本系統由 **26-27 年度首席導學風紀李創杰（LI Chuangjie Jacky）** 發起、設計並全程主導開發。

- **架構方向**：從單文件腳本到 7 層模組化架構，每一輪重構的決策均由他基於實際團隊需求做出。
- **品質標準**：62 項自動化測試、10 張架構圖、雙語完整文檔——這些標準並非 AI 自動產生，而是他對「專業級系統」的堅持。
- **持續迭代**：從最初的排班腳本，到引入 AI 解析、師徒配對、PDF 備份、DeepSeek 遷移、文檔重構——每一次重大升級都源於他對系統的不斷反思與改進要求。
- **人機協作主導**：在約 20 億 tokens 的 AI 輔助開發中，他始終是決策者——AI 提供建議與加速，他做出所有關鍵取捨。

> 沒有他的遠見、領導力與持續投入，這個專業級智能排班系統不會存在。
...
本系統從構思、架構設計、AI 輔助開發主導、到持續迭代優化，全程由李創杰發起並領導完成。
