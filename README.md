...
**目前可否在校外使用？**
遠端架構及唯一 workers.dev 入口已建立，但目前候選 Worker 仍待重新部署及完成管理員登入、登出、WebSocket、手機與 PDF 真人核對；在 `PROJECT_STATUS.md` 明確記錄部署後驗收通過前，不應把管理員遠端編輯視為已正式可用。完成後，未登入訪客只可閱讀產品導覽或指定已發布週表；獲准管理員才可經 Cloudflare Access 在同站解鎖 NiceGUI。一般使用者毋須安裝 WARP；WARP 只保留作維護後備。
