...
4. Generate and review the draft in the same post-by-week matrix used by the PDF. Click a cell, or press `Enter`／`F2`, to search Chinese names, select an eligible prefect, or explicitly create a vacancy.
5. Keep all edits on the page, then use **Review and save** to apply one atomic, version-checked batch. A blank search never deletes an assignment or creates a vacancy.
...
Drafts and official outputs distinguish four stable states: `assigned`, `vacant`, `room_closed`, and `day_closed`. Entering `X`, `×`, `空缺`, or `待安排` offers an explicit **Set as vacant** choice; empty input remains unfinished search only. Closing a day lists the assignments that will be removed before the batch is saved. Reopening it creates vacancies rather than silently restoring possibly stale assignments. A published roster must be withdrawn before its whole-day status can change. Guest exposes the same workflow against its in-memory fictional workspace only. See [Draft grid and whole-day closure](docs/ROSTER_DRAFT_EDITING.md) for keyboard, touch, conflict-recovery, PDF-parity, and migration details.
