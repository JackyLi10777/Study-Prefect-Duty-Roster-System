...
> **Mobile-first operations candidate (not deployed):** `codex/mobile-first-operations` makes the Android／iPhone weekly path week selection → closure／leave → rule summary → readiness → generation. The mobile drawer uses four dedicated rectangular state tiles rather than inheriting desktop rotary controls, so Sound and Appearance no longer create elliptical halos or vertical letter wrapping. While the drawer is open, bottom navigation is both visually hidden and semantically unavailable. Phone drafts show one weekday and six posts, weekday summaries, one shared bottom sheet, and one unsaved-change dock; desktop keeps the PDF-like matrix and all transaction, CAS, command-ID, fairness, and PDF semantics. See [mobile operations acceptance](docs/MOBILE_OPERATIONS_ACCEPTANCE.md). This paragraph describes source only; the generated status block remains the production authority.
...
5. Keep all edits on the page, including pointer／touch moves, swaps and undo／redo, then use **Review and save** to apply one atomic, version-checked batch. A blank search never deletes an assignment or creates a vacancy.
...
The rc53 source candidate distinguishes five stable states in drafts and official outputs: `assigned`, `vacant`, `room_closed`, `unavailable`, and `day_closed`. `unavailable` is an operator-set exception for one cell in one week; it is not an uncovered vacancy. Entering `X`, `×`, `空缺`, or `待安排` offers an explicit **Set as vacant** choice; empty input remains unfinished search only. Reopening a cell or day creates vacancies rather than silently restoring possibly stale assignments. A published roster must be withdrawn before a closure state can change. Guest exposes the same workflow against its in-memory fictional workspace only. See [Draft grid and whole-day closure](docs/ROSTER_DRAFT_EDITING.md) for keyboard, touch, move／swap, conflict-recovery, PDF-parity, and migration details. This paragraph is source-candidate documentation until formal release evidence updates the generated production status.
