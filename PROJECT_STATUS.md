...
The deployment decision is now a dedicated Windows 11 host whose NiceGUI origin always stays on `127.0.0.1`. `docs/WINDOWS_DEDICATED_HOST_SETUP.md` provides a zero-knowledge installation and operating procedure, including a Python 3.12 `.venv`, Task Scheduler startup, health checks, backup, updates, troubleshooting, and host replacement. Idempotent PowerShell scripts now prepare the Windows host, register startup, install cloudflared, activate the protected server profile, verify the unauthenticated Access redirect, and fail back to the previous local environment. `docs/CLOUDFLARE_REMOTE_ACCESS_SETUP.md` leaves only Cloudflare account, domain, named allow-list, AUD, and one-time Tunnel token decisions to the operator. Linux, Raspberry Pi, Docker, and true cloud hosting are not part of the selected production path.
...
- `pytest`: 221 passing tests, including PDF clean-export/crest controls, concurrent adjustment idempotency, fairness reconciliation, maintenance locking, stable-role browser fixtures, supply-chain gates, the anonymous Platform & Team summary boundary, read-only Engineering & Quality evidence centre, safe read-failure propagation, paired platform imagery, single-source bilingual feedback, enterprise-style Study Prefect Team operating model, official-role preservation, first-person authorial voice, and the established policy, workflow, persistence, backup, i18n, responsive, logging, deployment, repository-hygiene, music, architecture, and accessibility coverage.
