...
from roster.utils import (
    generate_pdf, generate_summary_pdf, generate_service_certificate, export_system_backup, import_system_backup,
...
def main():
    ...
    with tab_edit:
        ...
        if not edited_roster.equals(st.session_state.roster_df):
            st.warning(_t(
                "⚠️ 你有尚未套用的修改。請點擊下方「套用修改」按鈕來儲存變更。",
                "⚠️ You have unsaved changes. Click 'Apply Changes' below to save."
            ))
            col_apply, col_discard = st.columns([2, 1])
            with col_apply:
                if st.button(_t("✅ 套用修改 (Apply Changes)", "✅ Apply Changes"), type="primary", width="stretch", key="apply_manual_edits"):
                    set_state("roster_df", edited_roster)
                    trigger_backup_reminder()
                    st.rerun()
            with col_discard:
                if st.button(_t("↩️ 放棄修改", "↩️ Discard Changes"), width="stretch", key="discard_manual_edits"):
                    st.rerun()
    ...
    with st.form("leave_adjust_form", clear_on_submit=True):
        ...
        # 📋 Adjustment History — show past adjustments for transparency
    if st.session_state.get("adjustment_log"):
        with st.expander(
            _t(
                f"📋 調整歷史記錄（{len(st.session_state.adjustment_log)} 筆）",
                f"📋 Adjustment History ({len(st.session_state.adjustment_log)} entries)"
            ),
            expanded=False
        ):
            log_df = pd.DataFrame(st.session_state.adjustment_log)
            display_cols = {
                "time": _t("時間", "Time"),
                "day": _t("日期", "Day"),
                "role": _t("崗位", "Position"),
                "original": _t("原值班人", "Original"),
                "replacement": _t("替補", "Replacement"),
                "weight_revoked": _t("撤銷點數", "Points Revoked"),
            }
            log_display = log_df.rename(columns=display_cols)
            st.dataframe(log_display, width="stretch", hide_index=True)
            st.caption(_t(
                "💡 此記錄僅保留於當前工作階段。如需永久保存，請定期下載 JSON 備份。",
                "💡 This log is session-only. Download JSON backup regularly for permanent preservation."
            ))
...
"""
    ...
    if not st.session_state.master_report_df.empty:
        ...
        if st.button(get_text("generate_summary_button"), type="primary"):
            ...
            # Compute most neglected students from history data (robust, independent of trend analysis)
            _neglected = _t("無", "None")
            if st.session_state.get("history_loads"):
                _latest_loads = st.session_state.history_loads[-1]["loads"]
                if _latest_loads:
                    _sorted = sorted(_latest_loads.items(), key=lambda x: x[1])[:3]
                    _neg_names = [n for n, _ in _sorted]
                    _neglected = ", ".join(_neg_names)
            neg_str = _neglected
...
"""
            ...
            # 📄 Professional PDF Summary for annual review / external use
            col_pdf, _ = st.columns([1, 2])
            with col_pdf:
                if st.button(_t("📄 下載專業英文PDF摘要", "📄 Download Professional English PDF Summary"), type="secondary", width="stretch", key="summary_pdf"):
                    with st.spinner(_t("正在生成專業PDF摘要...", "Generating professional PDF summary...")):
                        try:
                            ...
                            if pdf_bytes:
                                st.download_button(
                                    _t("⬇️ 點擊下載PDF", "⬇️ Click to Download PDF"),
                                    pdf_bytes,
                                    f"SYSS_Summary_Report_{datetime.date.today().strftime('%Y%m%d')}.pdf",
                                    mime="application/pdf",
                                    width="stretch",
                                    key="summary_pdf_download"
                                )
                                st.success(_t("✅ PDF摘要已生成！適合年度審查會議或外部交流使用。", "✅ PDF summary generated! Suitable for annual review meetings or external sharing."))
                            else:
                                st.warning(_t("⚠️ PDF引擎未能初始化，請確認WeasyPrint已安裝。", "⚠️ PDF engine unavailable. Please verify WeasyPrint is installed."))
