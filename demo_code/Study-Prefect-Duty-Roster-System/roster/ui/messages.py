...
MESSAGES = {
    ...
    "success_adjust_complete": (
        "✅ 調整完成！{action_msg}\n\n累計點數與公平性圖表已即時更新。\n\n📄 **下一步：** 請向下滾動到「快捷匯出」區塊，點擊「重新匯出最新PDF」以下載更新後的值班表。PDF 快取已清除，匯出時會自動使用最新數據。",
...
Good luck! 🙏"""
    ...
    "adjustment_complete": (
        "✅ 調整完成！{action_msg}\n\n累計點數與公平性圖表已即時更新。\n\n📄 **下一步：** 請向下滾動到「快捷匯出」區塊，點擊「重新匯出最新PDF」以下載更新後的值班表。PDF 快取已清除，匯出時會自動使用最新數據。",
        "✅ Adjustment complete! {action_msg}\n\nCumulative points and fairness chart updated in real time.\n\n📄 **Next Step:** Scroll down to the Export section and click \"Re-export Latest PDF\" to download the updated roster. PDF cache has been cleared and will use the latest data."
    ),
    ...
    "pdf_restore_subheader": (
        ...
        "pdf_restore_language_guidance": (
        "📌 語言支援說明：本系統匯出的中文版和英文版 PDF 均內嵌完整備份數據。無論您使用哪個語言版本的 PDF，都可以成功還原。建議使用最近一次匯出的 PDF 以確保數據最新。",
        "📌 Language Support: Both Chinese and English PDFs exported by this system contain full backup data. You can restore from either language version. Use the most recently exported PDF for the latest data."
    ),
    "pdf_restore_what_restored": (
        "💡 還原內容包括：學生累計點數 (history_weight)、當週排班表 (roster_df)、請假記錄、全局負荷倍率、調整日誌 (adjustment_log)、師徒配對狀態 (needs_mentoring) 等所有動態數據。靜態數據（姓名、年級、職級）保持當前系統設定。",
        "💡 Restored data includes: cumulative student points (history_weight), weekly roster (roster_df), leave records, global load multiplier, adjustment log, mentoring status (needs_mentoring), and all other dynamic data. Static data (names, forms, roles) remain as currently configured."
    ),

    "pdf_restore_caption": (
        "上傳之前導出的完整 PDF 值班表（無需拆分頁面），系統將自動解析並還原所有風紀資料、排班記錄與師徒配對狀態。支援中文版和英文版 PDF，兩者均可使用。建議使用最近一次匯出的 PDF。",
        "Upload the complete exported PDF (no need to split pages). The system will automatically restore all prefect data, roster records, and mentoring pair status. Both Chinese and English PDFs are supported. Use the most recently exported PDF for best results."
    ),
}
