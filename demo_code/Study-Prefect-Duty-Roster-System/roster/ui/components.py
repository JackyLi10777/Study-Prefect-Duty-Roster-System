...
"""
        ...
        st.info(get_text("pdf_restore_language_guidance"))
        st.caption(get_text("pdf_restore_what_restored"))
        ...
        if uploaded_pdf:
            from roster.utils import parse_backup_from_pdf
...
def render_system_architecture_diagram(expander_label="System Architecture", caption_text="The layered modular architecture of this system. Arrows indicate module call relationships. See GitHub README for full documentation."):
    """
    Render system architecture Mermaid diagram in Streamlit using native st.markdown().
    
    Uses Streamlit's built-in Mermaid support (available since v1.26+) which renders
    reliably on Streamlit Cloud without external CDN dependencies. This replaces the
    previous st.components.v1.html() + CDN approach which failed on Cloud due to
    Content Security Policy restrictions and iframe sandboxing.
    """
    import streamlit as st
...
flowchart TD
    ...
    subgraph core["Core Layer 核心層"]
        ENGINE["Roster Engine\nengine.py"]
        POLICY["School Rules SSOT\nschool_policy.py"]
    end

    subgraph data["Data Layer 數據層"]
        STATE["State Management\nstate.py"]
        DEMO["Demo Roster\ndemo.py"]
    end

    subgraph ui["UI Layer UI 層"]
        COMP["Interface Components\ncomponents.py"]
        MSG["Bilingual Messages\nmessages.py"]
    end

    subgraph utils["Utility Layer 工具層"]
        PDF["PDF Generation\npdf.py"]
        BACKUP["Backup & Restore\nbackup.py"]
        IMP["Roster Import\nimporters.py"]
    end

    subgraph ai["AI Layer AI 層"]
        AI_P["DeepSeek Parser\nparser.py"]
    end

    subgraph errors["Exception Layer 異常層"]
...
"""
    ...
    with st.expander("🏗️ 系統架構圖（System Architecture）", expanded=False):
        ...
        st.markdown(f"```mermaid{mermaid_diagram}```")