...
# ====================== SUMMARY REPORT PDF (Annual Review / External Use) ======================
...
def generate_summary_pdf(
    ...
    """
    Generate a clean, professional PDF for the summary report.
    Designed for annual review meetings and external sharing.
    Uses the same WeasyPrint engine as the main roster PDF.
    """
    if not PDF_AVAILABLE:
        st.error("PDF engine not available. Please ensure WeasyPrint is installed.")
        return None
    ...
    logo_html = (
        f'<img src="data:image/png;base64,{logo_b64}" alt="School Logo" '
    ...
    _css = " ".join([
        ...
        ".header { background: #0F766E; color: white; padding: 24px 32px; border-radius: 8px; margin-bottom: 24px; }",
        ...
        ".kpi-card .value { font-size: 28px; font-weight: 700; color: #0F766E; }",
        ".section h2 { font-size: 18px; color: #0F766E; border-bottom: 2px solid #0F766E; padding-bottom: 6px; }",
        ".quote { background: #fef3c7; border-left: 4px solid #d97706; padding: 12px 16px; font-style: italic; margin: 16px 0; }",
        ".footer { margin-top: 32px; padding-top: 16px; border-top: 1px solid #e2e8f0; font-size: 10px; color: #94a3b8; text-align: center; }",
        ".summary-content { white-space: pre-wrap; font-size: 13px; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 8px; padding: 16px; }",
    ])
...
<body>
  ...
  <div class="header">
    ...
    <div class="subtitle">Study Prefect Duty Roster - Summary Report</div>
  </div>
  <div class="kpi-grid">
    <div class="kpi-card"><div class="label">Total Prefects</div><div class="value">{total_students}</div></div>
    <div class="kpi-card"><div class="label">Avg Cumulative Load</div><div class="value">{avg_load:.1f}</div><div class="unit">points</div></div>
    <div class="kpi-card"><div class="label">Fairness Index (Std Dev)</div><div class="value">{fairness_gap:.1f}</div><div class="unit">points</div></div>
  </div>
  <div class="section">
    <h2>Full Report</h2>
    <div class="summary-content">{summary_en}</div>
  </div>
  <div class="quote">
    "Whoever wants to become great among you must be your servant." - Mark 10:43<br>
    This system promotes fairness, equity, and a culture of service.
  </div>
  <div class="footer">
    Report Date: {report_date} | Generated by Sing Yin Study Prefect Duty Roster System<br>
    Powered by Grok and Grok Build - AI-assisted development with Codex + DeepSeek V4 Pro
  </div>
</body></html>"""
    try:
        return HTML(string=html).write_pdf()
