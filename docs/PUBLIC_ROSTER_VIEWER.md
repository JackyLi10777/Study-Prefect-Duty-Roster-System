...
連結在到期或撤銷前，任何持有完整 URL 的人都可查看；只應發給需要的人。已發布後請假調整不會修改已發出的密文內容，而是與正式調整在同一交易把所有較舊或可能已送達的 Viewer 快照排入撤銷，清除仍在本機 outbox 的解密材料，然後由介面立即嘗試 Worker 撤銷。若撤銷暫時失敗，調整仍已提交，操作者不得重複提交；畫面會保留待撤銷狀態及安全下一步。核對修正版後才建立及發送新連結。分享連結不能升級為 Guest 或 Admin；實際線上版本是否已包含此行為，以頁首生成狀態為準。
...
- [ ] 以虛構已發布週表建立 Viewer 連結後完成一次請假調整；舊版本在同一交易進入 durable revocation、立即嘗試撤銷，而精確重試不重複調整、不遺失撤銷收據，亦不撤銷其後建立的新版本連結。
