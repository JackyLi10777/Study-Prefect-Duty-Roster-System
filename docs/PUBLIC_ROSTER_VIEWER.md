...
> **復原層級：**第一個正式復原選擇是保留頁首生成狀態所列的現行程式，並使用同一狀態記錄的已驗證備份執行受控還原。資料庫目前位於 Alembic `0012`；任何較舊程式及 Worker 均只屬歷史來源，只有在隔離環境證明程式與目標快照相容後才能使用，不能只切換 origin 或 Worker。若舊程式只支援 pre-0012 schema，必須選取相容的 pre-0012 快照；rc42 從未部署，不能當成復原目標。
