...
任一項失敗都先停止正式寫入、記錄時間／route／裝置及非敏感畫面並登出，不要反覆提交表單。維護者依 [發布與交接手冊](RELEASE_HANDOVER.md) 判斷事故屬 origin、gateway 還是資料狀態：第一個復原選擇是保留頁首所列的目前程式，並以同一生成狀態記錄的已驗證正式備份執行受控還原。資料庫目前位於 Alembic `0012`；任何較舊程式都只屬歷史來源，必須先在隔離環境以相容快照通過 schema、完整性及應用驗證，不能只切換程式或 Worker。只有舊程式不支援 `0012` 時才選用相容的 pre-0012 快照。只看到 `/healthz` 200 不足以恢復使用。
...
Historically, clean `v1.2.0-rc.30` at `74b84f43786b00feb15b51a6270ff71c9430773f` passed 14／14 source-matched gates under fingerprint `15d155d8d745b14b574b08d793150c93aa77946e7d17a63030844c44adededbc` (894 Python tests, 3 motion, and 46 Worker contracts) and completed controlled Windows and Worker deployments. It was the first known verified clean recovery source at that time. Current production is identified only by the generated status at the top of this page; the first recovery path keeps that application and restores its recorded verified backup. Every older application is historical and must pass isolated schema, integrity, and application compatibility checks against the selected snapshot; use a pre-0012 snapshot only when the old application does not support `0012`. Supervised Head Study Prefect and teacher-advisor acceptance remains pending.
