...
> **rc53 來源候選提示：**`codex/rc53-spreadsheet-prefect-motion` 正在驗證指定週次單格不開放、草稿拖放／鍵盤移動與重做，以及名單搜尋／篩選／行內批次保存。這些功能在正式 release gate、schema `0014` 備份及隔離還原、合併和部署完成前，不屬於本頁頂部列出的 rc52 線上能力。
...
任一項失敗都先停止正式寫入、記錄時間／route／裝置及非敏感畫面並登出，不要反覆提交表單。維護者依 [發布與交接手冊](RELEASE_HANDOVER.md) 判斷事故屬 origin、gateway 還是資料狀態：第一個復原選擇是保留頁首所列的 rc52 程式與 Worker，並以同一生成狀態記錄的已驗證 schema-`0013` 正式備份執行受控還原。任何較舊程式都只屬歷史來源，必須先在隔離環境以相容快照通過 schema、完整性及應用驗證，不能只切換程式或 Worker。只看到 `/healthz` 200 不足以恢復使用。
...
Historically, clean `v1.2.0-rc.30` at `74b84f43786b00feb15b51a6270ff71c9430773f` passed 14／14 source-matched gates under fingerprint `15d155d8d745b14b574b08d793150c93aa77946e7d17a63030844c44adededbc` (894 Python tests, 3 motion, and 46 Worker contracts) and completed controlled Windows and Worker deployments. It was a verified recovery source at that time, but it is not a direct rollback target for the current schema. Current production is identified only by the generated status at the top of this page; the first recovery path keeps rc52 and restores its recorded verified schema-`0013` backup. Every older application is historical and must pass isolated schema, integrity, application and edge compatibility checks against the selected snapshot. Supervised Head Study Prefect and teacher-advisor acceptance remains pending.
