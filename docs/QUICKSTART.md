...
> **目前狀態（live rc9）：** `C:\SingYinRoster` 正運行 annotated tag `v1.2.0-rc.9`／commit `18a5c73`，`/healthz` 正常且 `/readyz` ready；canonical Worker 保留已驗證 live version `b13e5721-d1e8-4048-9885-ffb422fe2010`。統一 Guest 已在受保護設定下上線。`codex/service-weave-v1-2-editorial` 仍是尚未部署的 Service Weave 候選；它必須以自己的最終來源重新通過正式 gate、建立新備份並完成隔離還原及受控部署，才可取代 rc9。
...
The live `v1.2.0-rc.9` origin (`18a5c73`) and verified Worker `b13e5721-d1e8-4048-9885-ffb422fe2010` provide one canonical site with either a time-limited **Guest experience** using fictional in-memory data or **Admin login** through Cloudflare Access. A refresh in the same Guest tab may restore its latest signed revision, but a duplicated tab receives another workspace and logout, expiry, revocation, or origin restart invalidates the temporary token. Select **Log out** when finished because closing one tab does not clear every session state. `/view#…` remains the separate read-only published-roster link. `START_SING_YIN_ROSTER.cmd` is a local maintenance and recovery launcher only. It reuses an existing official service, chooses a free port from 8080–8099 when necessary, waits for HTTP readiness, and only then opens the browser. For a durable fictional rehearsal with backup/restore, use `START_PRACTICE_MODE.cmd`; the remote Guest workspace is intentionally temporary. Check `/readyz`, not only `/healthz`, before accepting official writes. The Service Weave editorial branch remains an undeployed candidate until its own complete gate and controlled rollout pass.
