...
2. 等待黑色視窗顯示「The system is ready」。瀏覽器只會在服務真正可以回應 HTTP 後才自動開啟。
3. 如瀏覽器沒有自動開啟，使用黑色視窗最後顯示的完整網址。
...
使用完畢後回到啟動器黑色視窗，按 `Ctrl+C`，或關閉該視窗。下次雙擊啟動檔時，啟動器會重新檢查服務狀態。
