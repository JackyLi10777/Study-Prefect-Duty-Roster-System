...
網站「交接指引」是這份矩陣的操作入口：桌面以三欄顯示名單／週表／備份準備度，手機改為依次堆疊；下方分開顯示機器證據與真人驗收，並把 H-01 至 H-21、A-01 至 A-04 分成七段真實工作演練。每段提供負責人、矩陣編號及直接工作頁入口，也可下載預設未勾選的受監督記錄。顏色以外亦有圖示、標題及說明文字，按鈕維持至少 44px 高；網站不保存或自行標記簽核，完成結果仍須由具名監督者按本文件及證據矩陣確認。
