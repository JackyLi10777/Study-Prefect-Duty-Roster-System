...
> **草稿試算表及全天停開候選（來源限定，未部署）：** `codex/roster-grid-day-closure-release` 已加入共用值班呈現模型、整批草稿修改及 additive migration `0013_roster_day_closures`。正式服務仍以頁首生成狀態為唯一真相；完成候選的正式驗證、備份、隔離還原、合併及受控發布前，不可聲稱此功能已在線上，也不可把 migration `0013` 當作正式主機現況。
...
9. 如目前正式 runtime 發現氣氛資產、Daily Verse、Assist 模式、可值班日、窄屏、鍵盤、焦點、主題或入口回歸，立即停止接受正式寫入並記錄 canonical URL、時間、裝置、route 及非敏感畫面。第一個復原選擇是保留當前程式並以本頁頂部所列的已驗證正式備份執行受控還原。若必須回復更舊程式，先確認目標程式實際支援的 Alembic revision；rc43／rc41 等 pre-0012 程式只可配對已驗證的 pre-0012 正式快照，禁止把 `0012` 資料庫交給舊程式。完成 checksum、公平、行數、restore-audit 及隔離副本的完整應用驗證後，才由事故負責人批准程式及必要的 Worker 切換。不可 `git reset --hard`、直接覆寫 C-host、只切換 task action 或留下未經證明的混合版本。
