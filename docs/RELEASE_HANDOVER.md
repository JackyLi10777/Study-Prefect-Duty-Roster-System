...
目前正式版本使用 Alembic `0013` 資料層、固定星期衝突拒絕、foreign keys、quick check、公平對帳及完整規模驗證，並完成頁首生成狀態所列的 exact-source 報告、已驗證備份、隔離還原及可寫 `/readyz`。第一個復原選擇是保留該現行程式並還原其已驗證 schema-`0013` 備份；任何舊程式都必須先以相容快照完成隔離驗證，不能作 code-only rollback。慢 SQL 診斷預設關閉；若暫時啟用，日誌只可保留指紋與耗時，不可包含 SQL 參數或值班內容。Worker 來源已更新，經 0% version smoke 後推廣至頁首所列的 100% canonical version；`wrangler.jsonc`、Access scope、Tunnel route、binding 與 secret-name contract 未更改。
...
> **rc54 整合候選（尚未部署）：**`0014_roster_slot_exceptions`、單格不開放、typed 草稿／名單 edit sessions、名單多列原子保存、Public Support 主題一致性及 mobile drawer 單一狀態仍須依本手冊完成 exact-source 15-gate、Alembic `0013` 已驗證備份（含摘要值及隔離還原證據）、隔離 migration／restore、Origin／Worker 分段發布與 canonical 驗收。不得把開發期綠燈或本段文字當成線上功能；正式狀態只看上方生成區塊。
...
2. 先登記未發布週的請假，核對「固定星期模式」或「每週靈活模式」，再選擇本週是否有「全天不開放」日，然後生成草稿。全天停開只屬指定週次，不會修改固定房間政策。
3. 以與 PDF 相同的矩陣核對每日崗位、中文姓名、請假及停開狀態。需要修改時直接點擊格子搜尋合資格人選，或明確設為空缺；rc53 來源候選亦支援暫存移動／交換、重做及指定單格不開放。所有修改先留在本頁，最後按「核對並保存」作一次交易。草稿 PDF 只可核對，不可張貼。
...
6. 如需瀏覽器直達查看，從已發布週表或「存取控制台」明確建立唯讀連結；等待操作完成，系統確認公開端已讀到同一份加密快照後才會顯示完整連結。只有後端提供真實事件時，介面才會顯示具名處理階段；否則只顯示不確定進度。只把完整連結發給需要查看的人。
7. 如發布後有人請假，使用「請假調整」；不要改動歷史資料或直接覆寫 SQLite 檔案。完成收據會顯示扣回／轉移點數、週表新版本、帳本對帳及備份結果。立即用收據上的入口重新匯出及分享修正版；如使用 Viewer，建立新連結並撤銷舊連結。

本頁頂部生成狀態所識別的正式環境沿用四個穩定狀態；rc53 來源候選擴充為五個：`assigned`（已安排）、`vacant`（空缺／待安排）、`room_closed`（個別房間按政策不開放）、`unavailable`（本週指定單格不開放）及 `day_closed`（本週指定日期全天不開放）。優先序固定為 `day_closed > room_closed > unavailable > assigned/vacant`，空白輸入永遠不是已確認狀態。關閉已有安排的格子或日子會清除相應草稿安排；日後重新開放只顯示空缺，不會靜默恢復舊安排。已發布週表不可直接改停開狀態；先撤回整週並完成公平補償及分享撤銷，再建立正確草稿。Guest 只在記憶體內以虛構資料演練相同行為。完整操作、已部署 whole-day migration 及來源候選 `0014` 交接見[草稿試算表與全天停開操作](ROSTER_DRAFT_EDITING.md)。

格子修改、原子交換及星期停開只在「核對並保存」時以週表版本和命令 ID 整批提交；任一驗證失敗時整批不保存。若另一分頁已更新同一草稿，必須保留目前輸入並提供重新載入、比較及重新套用，禁止最後寫入者靜默覆蓋。網頁、繁中／英文 PDF 及公開分享須由同一 `RosterSchedulePresentation` 產生；Guest 只可在有限期虛構記憶體工作區演練，不得建立正式備份、帳本或分享。

工作台操作在 140ms 內完成時不顯示載入視窗；超過 140ms 且無可量度進度時只顯示誠實的 indeterminate 狀態。只有後端提供真實事件時才可顯示階段名稱，沒有真實 `completed／total` 就不得顯示百分比。完成結果不得為了動畫而延遲。身份導向及跨頁細軌仍採各自的 150ms 門檻，兩種契約不可混寫。
...
名單新增／修改／停用，以及生成前請假登記／取消，均可能同時建立本機快照。按下操作後，請等候操作完成，不要重複點擊或關閉分頁；只有後端提供真實事件時才會顯示具名階段，否則使用不確定進度。漏填中文姓名、班別、可值班日或替補選擇時，系統會先把焦點帶回相關欄位，不會開始寫入；週開始日期不是星期一亦會先提示修正。生成前請假、草稿修改、發布後請假調整及撤回原因均屬選填；留空不會削弱版本核對、公平對帳、審計或備份。停用風紀前必須閱讀確認：停用不是刪除，既有週表、公平帳本及審計紀錄仍會保留，而且介面沒有即時復原功能。

如另一個分頁已修改同一位風紀或同一份草稿，系統會拒絕較舊版本，不會用舊畫面覆蓋新資料。不要再次按儲存；依提示重新載入、核對最新內容，再重做仍然需要的改動。外觀控制只有單一淺色／深色按鈕；未設定時跟隨系統，第一次操作會保存目前解析結果的相反模式，其後只在明確淺色與深色間切換。外觀和提示音會在原頁即時切換，不會令表單重載。語言按鈕顯示目的語言本名 English／EN 或繁體中文／繁中；語言切換需要重載，因此有未儲存輸入時會先詢問是否離開。
...
「已編排時數」只按目前 `DUTY_SERVICE_TIME_WINDOWS` 對最終值班安排作排程換算；所有崗位的房間開放顯示及實際當值時段均為 15:40–17:00。`ROOM_OPENING_TIME_WINDOWS` 與服務時段仍為兩項獨立政策，避免日後調整房間開放時間時意外改變服務時數。系統沒有簽到／完成服務紀錄，因此這個數字**不是出席證明、完成服務時數、個人表現評核或服務證書**。如日後需要正式證書，必須另行建立已核實的出席及批准流程，不能改稱現有排程資料。
...
再次確認後，才可把有限 TXT／JSON／PNG 證據保存到主機本機收件匣；Public 及 Viewer
只可經短效簽署能力提交受限純文字；成功回傳 `INC-…`，網絡、origin 或收件匣失敗時保留本分頁 `FB-…`。Guest 一律只在瀏覽器建立報告。支援記錄不屬於排班交易、正式
...
登入入口每次開啟都會以 50% 音量嘗試播放歡迎音樂一次，其後保留所有明確選擇的音量，包括 25%。未選擇聲音偏好時，管理員或訪客按鈕本身會同步重試播放，並在成功、拒絕、例外或短時限屆滿後只前往所選身份一次；音樂故障不是登入故障。「預設：開啟音樂」與「安靜繼續」只是可選偏好／復原控制，手動暫停或安靜選擇只在目前入口停留期間優先。其他頁面操作不會暗中重試，瀏覽器政策仍是最終依據。登入後的本機情境音樂保留獨立的工作台偏好：新版結構只會把仍等於舊版精確 24% 或 35% 預設的瀏覽器升級一次，首席導學風紀可在耳機控制／設定關閉跨頁自動播放。同一首本機歌曲如適用於跳轉前後兩頁，系統會在目前瀏覽器 session 延續播放位置及播放／暫停狀態，不會從頭開始；換歌或關閉瀏覽器 session 則不沿用。音樂狀態不影響名單、排班、公平、備份或 PDF。
...
rc27 的正式部署證據固定為 tag `v1.2.0-rc.27`／commit `c4c728aa41c9b0122aaa2c015b3cc38e246db43d`／fingerprint `71c8011c24dd0e05250c94cdc3b424bbc44a66b10ef5364f9f19b54b52c19c9c`。正式備份 `20260726-202210-704981-manual_verified_backup.sqlite3`、SHA-256 `a700a033159f0a940710d2c48b2fd1e5de869877e21b6c5e592605c613eb16f1`、公平對帳、行數核對、還原審計及隔離還原已通過；Worker source／設定沒有改動，故仍為 `76a23134-8355-4e25-bbba-abf17c6918c5`。第一級 origin 回退是 rc26／`248955cb3300bfbe092b05036632991524d824cd`。

### 歷史 rc26 第一級回退證據

rc26 的正式部署證據固定為 tag `v1.2.0-rc.26`／commit `248955cb3300bfbe092b05036632991524d824cd`／fingerprint `5da902307e2d717a75c93e100ba9860eb7e6dd9c35dc42d4a1477bd3304de5e7`。正式備份 `20260726-134020-201449-manual_verified_backup.sqlite3`、SHA-256 `51d116b4cb4a4ab5c6b713f9070b441c73e30eb6b7a75aaa105a53aaad582117` 及其隔離還原已通過；Worker source／設定沒有改動，故仍為 `76a23134-8355-4e25-bbba-abf17c6918c5`。這是 rc27 的第一級 origin 回退；rc24／`8d709f9b0b4e69fe38f7237ef2f473c27ff848fc` 保留為次級已驗證基線。以下 rc21／rc20 內容只保留作歷史程序參考。
...
3. 當時保存 rc20／`e3d84858` 及 Worker `f780feb2-671a-4feb-b6f6-b7f9d5b31e89` 作不可變版本，另保留 rc18／`fd504a8` 作當時的第一級回退。這是歷史程序；目前必須使用上方 rc26／rc24 層級。
...
9. 如目前正式 runtime 發現氣氛資產、Daily Verse、Assist 模式、可值班日、窄屏、鍵盤、焦點、主題或入口回歸，立即停止接受正式寫入並記錄 canonical URL、時間、裝置、route 及非敏感畫面。第一個復原選擇是保留當前程式並以本頁頂部所列的已驗證 schema-`0013` 正式備份執行受控還原。若必須回復更舊程式，先確認目標程式實際支援的 Alembic revision，並配對相容的已驗證快照；禁止把 schema `0013` 資料庫直接交給不支援它的舊程式。完成 checksum、公平、行數、restore-audit 及隔離副本的完整應用驗證後，才由事故負責人批准程式及必要的 Worker 切換。不可 `git reset --hard`、直接覆寫 C-host、只切換 task action 或留下未經證明的混合版本。
10. 復原後重新核對 task marker、bundle fingerprint、Alembic revision、工作排程 owner、受保護 loopback endpoint、`/healthz`、`/readyz`／`writeReady=true`、無 maintenance／recovery／pending backup obligation，以及 canonical Public／Guest／Admin／Viewer／WebSocket／登出。不可只以 inactive `C:\SingYinRoster` Git checkout 的 HEAD 判斷已運行版本。只有 origin、gateway 的來源與健康、使用者流程及資料狀態全部一致才恢復日常操作；若不能證明復原完成，保持 maintenance／唯讀並交由 IT 處理。
...
網站「交接指引」是這份矩陣的操作入口：桌面以三欄顯示名單／週表／備份準備度，手機改為依次堆疊；下方分開顯示機器證據與真人驗收，並把 H-01 至 H-22、A-01 至 A-04 分成七段真實工作演練。每段提供負責人、矩陣編號及直接工作頁入口，也可下載預設未勾選的受監督記錄。顏色以外亦有圖示、標題及說明文字，按鈕維持至少 44px 高；網站不保存或自行標記簽核，完成結果仍須由具名監督者按本文件及證據矩陣確認。

### 首席導學風紀

- [ ] 在 Admin／Guest、桌面／手機、淺／深及 reduced-motion 下核對入口與等待狀態：身份導向及跨頁細軌在 150ms 前不閃爍；工作台操作在 140ms 內不顯示載入視窗，超過 140ms 只顯示真實 indeterminate 階段，且不得加入最低等待時間；8s 慢速狀態可安全重試，返回後沒有卡住 busy；沒有 `completed／total` 時不得顯示百分比或自動重送。
...
- [ ] 發布後請假調整只提供合資格替補，並在帳本和審計中保留理由；完成收據準確列出原值班者扣回、替補者等額加回或保留空缺、更新後版本、備份狀態及舊 Viewer 撤銷結果，並提醒舊 PDF 不會自行更新。撤銷暫時失敗時不得重複提交調整，只重試 durable revocation。
