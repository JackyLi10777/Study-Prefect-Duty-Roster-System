...
> **rc23 已完成 exact-source 機器驗證，但尚未上線：**commit `f0fc91e7e24cf6e1d58a5065bbeba42c4708322a`／runtime fingerprint `2b9c3f7cb09d0614d71210d8daea16ab3d719c7e8827470a6b2cd1a79e20072b` 已通過 14／14 正式 gate，包括完整 Python suite、3 motion、40 Worker contract、桌面／手機／平板、效能、Admin／Guest、雙語 PDF、完整寫入／公平／交接／隔離還原及備份失敗復原。瀏覽器阻止迎賓音樂時，「安靜繼續」會保留並前往原定入口。Admin 支援事件只在明確同意後寫入隔離本機 inbox；檢視時會拒絕 manifest 外檔案、缺漏檔案或 SHA-256 不符，過期清理亦不會因單一損壞目錄中斷。Guest／Public／Viewer 支援摘要不作應用網絡寫入並在重新整理後消失。此證據不等於部署；建立 immutable tag、合併 protected main、正式備份／還原、Windows／Worker 配對切換及 canonical smoke 前，仍須以 rc21 作日常及回退基線。
