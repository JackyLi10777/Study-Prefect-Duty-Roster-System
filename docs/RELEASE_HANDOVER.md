...
9. 如目前 rc43 runtime 發現氣氛資產、Daily Verse、Assist 模式、可值班日、窄屏、鍵盤、焦點、主題或入口回歸，立即停止接受正式寫入並記錄 canonical URL、時間、裝置、route 及非敏感畫面。由受控 clean-bundle 流程把 Windows origin 回復至 rc41／`74072b0175ff64807312a8cc5b9cd016b6628210`，並由受控 Worker rollback 把 100% 流量回復至 `610092f6-59d4-4fd4-ab3a-3fbf1dd2c64e`。更舊來源只在 rc41 不能安全恢復且事故負責人批准時使用。不可 `git reset --hard`、直接覆寫 C-host 或留下未經證明的混合版本。
10. 回退後重新核對 task marker 確為 rc41／`74072b0175ff64807312a8cc5b9cd016b6628210`、Worker `610092f6-59d4-4fd4-ab3a-3fbf1dd2c64e` 承接 100% traffic、工作排程 owner、受保護 loopback endpoint、`/healthz`、`/readyz`／`writeReady=true`、無 maintenance／recovery／pending backup obligation，以及 canonical Public／Guest／Admin／Viewer／WebSocket／登出。不可只以 inactive `C:\SingYinRoster` Git checkout 的 HEAD 判斷已運行版本。只有 origin 與 Worker 的來源、健康、路由、使用者流程及資料狀態全部一致才恢復日常操作；若不能證明回退完成，保持 maintenance／唯讀並交由 IT 處理。
