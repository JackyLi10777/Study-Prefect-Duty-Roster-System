...
> **草稿試算表及全天停開候選（來源限定，未部署）：** `codex/roster-grid-day-closure` 已加入共用值班呈現模型、整批草稿修改及 additive migration `0013_roster_day_closures`。正式服務仍以頁首生成狀態為唯一真相；完成候選的正式驗證、備份、隔離還原、合併及受控發布前，不可聲稱此功能已在線上，也不可把 migration `0013` 當作正式主機現況。
...
- [ ] 在 Admin／Guest、桌面／手機、淺／深及 reduced-motion 下核對入口與等待狀態：150ms 前不閃爍進度，延遲時角色文案正確，8s 慢速狀態可安全重試，返回後沒有卡住 busy；正式操作只顯示真實階段，沒有 `completed／total` 時不得顯示百分比或自動重送。
