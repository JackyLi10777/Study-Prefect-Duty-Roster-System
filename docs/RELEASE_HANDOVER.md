...
3. 以與 PDF 相同的矩陣核對每日崗位、中文姓名、請假及停開狀態。需要修改時直接點擊格子搜尋合資格人選，或明確設為空缺；所有修改先留在本頁，最後按「核對並保存」作一次交易。草稿 PDF 只可核對，不可張貼。
...
草稿及交接文件必須沿用四個穩定狀態：`assigned`（已安排）、`vacant`（空缺／待安排）、`room_closed`（個別房間按政策不開放）及 `day_closed`（本週指定日期全天不開放）。空白輸入永遠不是已確認狀態。關閉已有安排的日子會清除該日草稿安排；日後重新開放只顯示空缺，不會靜默恢復舊安排。已發布週表不可直接改停開日；先撤回整週並完成公平補償及分享撤銷，再建立正確草稿。Guest 只在記憶體內以虛構資料演練相同行為。完整操作及 migration `0013` 交接見[草稿試算表與全天停開操作](ROSTER_DRAFT_EDITING.md)。
...
工作台操作在 140ms 內完成時不顯示載入視窗；超過 140ms 才顯示誠實的 indeterminate 階段。完成結果不得為了動畫而延遲，沒有真實 `completed／total` 就不得顯示百分比。身份導向及跨頁細軌仍採各自的 150ms 門檻，兩種契約不可混寫。
