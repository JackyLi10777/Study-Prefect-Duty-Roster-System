...
> **目前狀態（live rc21）：** `C:\SingYinRoster` 已固定於不可變標籤 `v1.2.0-rc.21`／`f7df4d0170e6bacd65340cc893992a17b5ed4aed`；第一級回退 `v1.2.0-rc.20`／`e3d84858abfe23714929a87c4bcf76e55999ce7c`，並完成正式資料快照、隔離還原及受控切換。`Sing Yin Roster Host` 由非管理員 `SingYinRosterSvc` 帳戶運行；實際 loopback endpoint 由受保護 `.env` 的 `SING_YIN_HOST`／`SING_YIN_PORT` 決定，現為 `127.0.0.1:8080`。canonical Worker version `f780feb2-671a-4feb-b6f6-b7f9d5b31e89` 正承接 100% 流量；`cloudflared` Windows service 已恢復為 Running／Automatic，Tunnel、Public、Guest、Access 轉向、VPC health 及 canonical 桌面／手機 rendered smoke 已核對。正式備份及隔離還原已通過。
>
