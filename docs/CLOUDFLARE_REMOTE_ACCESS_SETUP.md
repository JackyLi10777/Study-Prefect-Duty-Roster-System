...
Service Weave rc18 is the live controlled release; rc19 is an undeployed mobile／tablet／accessibility candidate. Preserve the protected Guest setting while validating rc19 in isolation; require a source-matched report, fresh verified backup, isolated restore, healthy `/healthz` and `/readyz`, a deliberate matching Worker deployment, and supervised browser acceptance. If the rc19 rollout fails, restore the recorded rc17 host bundle and Worker version shown in the rollback section.
