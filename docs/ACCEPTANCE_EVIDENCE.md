...
| H-20 | 新週預設固定星期模式；固定模式維持 AHP 原有星期，靈活模式只在可當值日輪換並在可行情況避免重複上週同日 | `test_assist_assignment_modes.py`、`test_assist_mode_persistence.py`、`test_assist_mode_guardrails.py`、`test_assist_assignment_mode_ui_contract.py` 鎖定 AHP-only、可當值日、請假、固定擁有者、靈活輪換、模式持久化、重複固定日拒絕及 UI 契約 | 以虛構 AHP 連續生成兩週固定模式及兩週靈活模式；核對固定星期不漂移、請假只替補該次、靈活模式有輪換、不可當值日從不被使用，並確認中英文模式說明容易理解 |
