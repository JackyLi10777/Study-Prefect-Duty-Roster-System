...
| H-12 | 交接包含 SQLite、manifest、說明；離機副本只接受批准的 BitLocker 外置媒體並由 copied bundle 還原 | handover／backup tests；`test_offsite_recovery.py`、CLI 及 Windows wrapper contract 覆蓋 exact pair、receipt、tamper、target 及刪除原主機資料後的隔離還原 | 在真實 BitLocker USB／SD 完成匯出、安全退出、離線保存及 replacement-location drill；記錄 RPO／RTO 與分離保管責任 |
