...
> **發布界線（2026-07-27）：** live `v1.2.0-rc.30`／`74b84f43786b00feb15b51a6270ff71c9430773f` 與 Worker `11763f08-d40d-46d5-93dc-5ca2599d4154` 是現行已部署基線；第一級 origin 回退是 rc27／`c4c728aa41c9b0122aaa2c015b3cc38e246db43d`，Worker 的立即回退版本是 `d7b51f21-7692-418d-866c-034c2c57292d`。rc30 的 296-file runtime 指紋 `15d155d8d745b14b574b08d793150c93aa77946e7d17a63030844c44adededbc` 通過 14／14 正式 gate，包括 894 項 Python、3 個 motion 及 46 個 Worker contract，並完成受控 Windows 切換、正式備份、隔離還原、0% Worker smoke、100% promotion、origin health／readiness 與 canonical rendered checks。機器與線上證據不能代替真人驗收，後者保持未完成。
>
> **rc31 來源候選（未上線）：** `codex/rc31-unified-theme-controls` 把可見 System／Light／Dark 三選一改為單一淺色／深色控制；`system` 只保留為偏好缺失／未設定時的初始化狀態。未設定時跟隨裝置系統，第一次操作儲存目前解析結果的相反模式，其後只在明確淺色與深色間切換。完整本機 Python suite 已通過 901／901；`scripts/verify_rc31_theme_controls.py` 使用臨時 SQLite、備份、日誌及 NiceGUI storage，通過 Admin／Guest × desktop／mobile × OS-light／OS-dark 共 8／8 個 Chromium 情境、24 張截圖，且沒有 console、page 或 server error；Worker contract 維持 46／46。這個 verifier 已成為正式 release 的第 15 個 fingerprint-bound gate。rc30 仍是現行已部署基線；rc31 尚未建立 protected-main commit、正式來源指紋、備份、Worker 或部署證據，首席導學風紀及教師顧問真人驗收亦保持未完成。
>
