...
1. 維護者先執行 `python -X utf8 scripts\verify_release_candidate.py`，確認 JSON 報告目前十項檢查均為 `pass`；其中 `repository_hygiene` 必須證明有真正 commit 歷史、無已追蹤敏感檔及 ignore 契約完整，`security_gates` 必須通過依賴、靜態程式及秘密掃描，而 `verify_runtime_performance` 必須證明字體完成後的冷載、強制 GC 後 heap／DOM／listener 增長及手機 overflow 均在門檻內。
...
| H-03 | 非星期一、缺請假／替補／修改原因在本頁修復 | `test_roster_persistence.py`; `test_accessibility.py`; write pipeline 檢查無進度／無寫入 | 用鍵盤及滑鼠各操作一次，確認焦點位置自然 |
...
- 修改排班政策、交易、備份、PDF、語言、路由復原或發布驗證器後，必須重新執行完整發布候選驗證。
