...
>
> **rc31 綜合來源候選（已凍結、未上線）：** `codex/rc31-unified-theme-controls` 已包含完整普通房間求解及嚴格 slot／weight 驗證、Admin／Guest mode-and-session-bound 下載、手機抽屜同步、全業務寫入 admission、restore-marker diagnostic-only 啟動，以及精確 database／manifest 備份、還原和交接驗證。公開入口的外觀在未設定時跟隨系統；刻意進入 Admin／Guest 時，只可暫存明確 `light`／`dark` 提示 120 秒，Worker 核對後才把它放入簽署 session 及 request-bound principal，建立 session 時清除暫存 cookie，既有目的地偏好優先。Guest／Admin 單檔上限分別為 5／64 MiB，registry 128 MiB 並保留 64 MiB／16 票證予 Admin。`/readyz` 還須有 `workflowInitialized=true`；diagnostic-only 程序即使 marker 被移除亦維持不可寫。較早 901／929 Python 及八個直接 Chromium 情境只屬歷史切片；凍結的 297 個可部署來源檔案已以指紋 `7f405269322e67ddc1fdfd5dde004af5079b315725487303fbecd8e1c0954042` 通過 15／15 正式 `--release` 閘門，包括完整 Python、48／48 Worker contracts、16 個本機渲染 Public→Admin／Guest 主題聯動情境、完整寫入／雙語 PDF、備份失敗及隔離還原。尚待完成的是 protected-main 合併與標籤、正式備份與隔離還原、相配部署、canonical 線上 checks 及真人驗收。乾淨 rc30＋`11763f08…` 仍是最近完整驗證的乾淨基線；active pair 來源待對帳。
>
