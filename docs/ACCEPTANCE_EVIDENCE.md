...
> **發布界線（2026-07-26）：** live `v1.2.0-rc.24`／`8d709f9b0b4e69fe38f7237ef2f473c27ff848fc` 與 Worker `76a23134-8355-4e25-bbba-abf17c6918c5` 是現行已部署基線；第一級回退是 `v1.2.0-rc.21`／`f7df4d0170e6bacd65340cc893992a17b5ed4aed`，rc20／`e3d84858abfe23714929a87c4bcf76e55999ce7c` 是次級已驗證基線。rc24 的 296-file runtime 指紋 `a6a1f4641f0eafa54fb740eb57f9173febc651ab0f11e3cfefcbe4c6ce38f477` 通過 14／14 正式 gate，包括完整 Python suite、3 個 motion 及 41 個 Worker contract，並完成受控 Windows 切換、正式備份、隔離還原、Worker staged rollout 與 canonical Public／Guest／Viewer smoke。Public `/support` 保持瀏覽器暫存，已驗證 Guest `/support` 進入 NiceGUI 工作台；機器與線上證據不能代替真人驗收，後者保持未完成。
...
| H-18 | v1.2 Guest 與 Admin 使用相同 NiceGUI 路由，但 Guest 只操作每分頁隔離的虛構記憶體 workspace；重新整理可還原最新合法 token，複製／篡改／過期／重啟後不可重播 | rc20 的 `verify_unified_guest_ui` 正式 gate 配合 `test_guest_workspace.py`、`test_guest_adapter.py`、`test_guest_downloads.py`、`test_guest_snapshot_bridge.py` 驗證相同路由、簽署、綁定、nonce、revision、重播拒絕、下載及 `sessionStorage` 邊界。報告已綁定 rc20 source fingerprint；這不代表候選已部署 | v1.2 部署後在實體手機完成訪客請假→生成→修改→示範發布→雙語 PDF／JSON→請假調整；核對中文姓名、`DEMO`、30 分鐘、兩分頁隔離、重新整理、登出及 Network／正式資料邊界 |
