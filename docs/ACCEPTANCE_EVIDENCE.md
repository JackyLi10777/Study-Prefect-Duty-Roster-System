...
> **線上來源真相（2026-07-28）：**目前 Windows runtime operational，但 rc30 checkout 已觀察到 73 tracked 修改及 3 untracked 項目；canonical Worker `a2e3ad14-d191-4ffc-85e4-eda40e42e5ed` 未建立來源歸屬。故 active pair 不可稱為 exact rc30 或指紋相符。乾淨 rc30＋`11763f08-d40d-46d5-93dc-5ca2599d4154` 是最近完整驗證的乾淨組合；`11763f08…` 是立即已知已驗證 edge 回退，`d7b51f21…` 是更早歷史版本。rc31 未凍結、未標記、未部署；真人驗收仍未完成。下文舊「live／current rc30」標籤是歷史證據名稱，不代表目前來源狀態。

本文件把機器驗證與真人驗收分開。`logs/release-candidate-report.json` 顯示 `pass`，只代表下列自動化證據在隔離虛構資料中通過；它不代表實際名單、學校做法、專用電腦、加密離機位置或外部存取決定已獲真人批准。

> **歷史 rc30 乾淨發布證據（2026-07-27）：** `v1.2.0-rc.30`／`74b84f43786b00feb15b51a6270ff71c9430773f` 與 Worker `11763f08-d40d-46d5-93dc-5ca2599d4154` 是最近完整驗證的乾淨組合，不代表目前來源漂移的 active pair。`11763f08…` 是立即已知已驗證 edge 回退；`d7b51f21…` 與 rc27 是更早 edge／origin 復原歷史。rc30 的 296-file runtime 指紋 `15d155d8d745b14b574b08d793150c93aa77946e7d17a63030844c44adededbc` 通過 14／14 正式 gate，包括 894 項 Python、3 個 motion 及 46 個 Worker contract，並完成受控 Windows 切換、正式備份、隔離還原、0% Worker smoke、100% promotion、origin health／readiness 與 canonical rendered checks。機器與線上證據不能代替真人驗收，後者保持未完成。
>
> **rc31 綜合來源候選（未上線）：** `codex/rc31-unified-theme-controls` 已包含完整普通房間求解及嚴格 slot／weight 驗證、Admin／Guest mode-and-session-bound 下載、手機抽屜同步、全業務寫入 admission、restore-marker diagnostic-only 啟動，以及精確 database／manifest 備份、還原和交接驗證。公開入口的外觀在未設定時跟隨系統；刻意進入 Admin／Guest 時，只可暫存明確 `light`／`dark` 提示 120 秒，Worker 核對後才把它放入簽署 session 及 request-bound principal，建立 session 時清除暫存 cookie，既有目的地偏好優先。Guest／Admin 單檔上限分別為 5／64 MiB，registry 128 MiB 並保留 64 MiB／16 票證予 Admin。`/readyz` 還須有 `workflowInitialized=true`；diagnostic-only 程序即使 marker 被移除亦維持不可寫。較早 901／929 Python 及八個直接 Chromium 情境只屬歷史切片；目前聚焦回歸、48／48 Worker contracts 及 16 個本機渲染 Public→Admin／Guest 主題聯動情境通過。凍結來源完整測試、正式 `--release`、protected-main commit／tag／指紋、正式備份與隔離還原、相配部署、canonical 線上 checks 及真人驗收仍待完成。乾淨 rc30＋`11763f08…` 仍是最近完整驗證的乾淨基線；active pair 來源待對帳。
>
...
| H-05 | 302／303／202 人數、開放日、名冊「可值班日」、同日不重複、不連續；普通房間求解只能返回完整週表或受控無解，不得留下部分安排或錯誤點數 | `test_roster_policy.py::test_generated_roster_preserves_non_negotiable_rules`; `test_assist_mode_guardrails.py`; Room 202 closure、greedy-dead-end、完整 slot multiplicity 及 duty-weight rejection 候選測試。rc31 的 exact-current-source 正式執行仍待完成 | 抽查一個實際週，確認未選的不可當值日從未被使用，所有必需格完整且點數正確，並確認校務安排沒有臨時政策變更 |
...
| H-14 | 同一網站在手機、200% zoom、tablet 及 desktop 保持同一產品並真正 reflow，讀取／焦點順序完整；手機抽屜以實際渲染狀態同步 ARIA、inert 及 focus，離頁不殘留 listener／RAF | rc27 source-matched report 已包含 `verify_nicegui_mobile.py`、`verify_nicegui_ui.py`、`test_mobile_layout.py`、`test_accessibility.py`、`test_motion_system.py` 及單一裝置矩陣。rc31 另有 rendered-state settle／快速切換／cleanup 候選覆蓋，但須由 exact-current-source 報告及 20 次路由循環重新證明，不能沿用固定延遲或較早候選結果 | 在實體 iPhone Safari 及 Android Chrome 核對 200% zoom、軟鍵盤後焦點欄位、跨頁 main focus、快速開關 More 抽屜、兩個 themes、reduced motion、forced colours、旋轉、瀏海／home indicator；另以實際 tablet 及 desktop 抽查 shell／欄位共存，不另建 `/mobile` |
...
| A-02 | 最近備份可驗證及還原；還原只接受精確 database／manifest 配對，拒絕 sidecar、非 object／損壞 manifest、待完成 backup obligation 及未知／未來 migration，並在成功後回到 write-ready | 既有 strict readiness、`test_backup_restore.py`、write pipeline 第二隔離資料庫及 maintenance lease；rc31 候選另覆蓋來源／staged substitution、DB／manifest digest、`0007` 支援鏈隔離遷移、current schema／FK／fairness、失敗自動 rollback、exact-pair handover ZIP 和 diagnostic-only marker。這組證據仍待 exact-current-source 正式執行 | 在非正式副本完成一次受監督演練並記錄日期；確認維護期間所有業務寫入不能進入，成功後 `/readyz` 恢復 write-ready；另確認 durable marker 啟動只進 diagnostic-only，而不是手動刪除 marker |
