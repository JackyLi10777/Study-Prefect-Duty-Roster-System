...
- `Escape` 取消目前編輯；`Ctrl+Z`（macOS 為 `Cmd+Z`）撤銷最近一次本頁修改。
...
「核對並保存」會以週表版本及命令 ID 執行一個原子交易，重新驗證職務、可值班日、請假、同日重複、連續當值及固定星期 Assist. 規則，並建立命令收據、審計及備份義務。任一格失敗時，整批不會部分保存。
...
- 網頁、繁中／英文 PDF 及公開分享均由同一呈現模型產生，保持星期、實際日期、英文崗位名稱、服務時間、中文姓名及四種狀態一致。
- PDF 不顯示內部停開原因；全天停開以完整星期欄清楚標示，固定房間停開仍只標示相應房間格。

## 維護者交接 / Maintainer handover

- Alembic revision: `0013_roster_day_closures`（additive）。
- `roster_day_closures` 使用穩定星期代碼，限制每個週表／星期唯一；翻譯文字不是資料庫鍵值。
- 公開型別：`WeekScheduleOverrides`、`DraftCellEdit`、`DraftDayEdit`。
- 交易入口：`apply_draft_patch(roster_week_id, expected_version, cell_edits, day_edits, command_id)`。
- 共用輸出：`RosterSchedulePresentation`，狀態只限 `assigned / vacant / room_closed / day_closed`。
- migration 上線前必須完成備份及隔離還原；migration `0013` 後，舊程式不能作 code-only rollback。

## 人工驗收清單 / Acceptance checklist

- [ ] 能清楚分辨空白、空缺、房間不開放及全天不開放。
- [ ] 中文單字可找到完整中文姓名；下拉只顯示合資格人選。
- [ ] `X`／`×`／`空缺`／`待安排` 只在明確確認後建立空缺。
- [ ] 原子交換、撤銷、取消全部及整批保存按預期運作。
