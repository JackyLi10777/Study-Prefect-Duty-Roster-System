...
| Admin read／write、Guest isolation／capacity、Worker service binding／WebSocket、下載／backup／outbox、Viewer publish／decrypt 混合負載 | `CODE_ACCEPTANCE_REVIEW.md` 定義何時量度，`ACCEPTANCE_EVIDENCE.md` 擁有驗收映射，結果進日期化 `audits/`；只有正式部署身份改變才更新 current-release JSON |
