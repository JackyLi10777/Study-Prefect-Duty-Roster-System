...
> replaces the visible System／Light／Dark selector with one binary Light／Dark
> control. `system` exists only while the preference is missing or unset: the
> resolved appearance follows the operating system until first activation,
> which stores the opposite resolved mode; later activations alternate explicit
> Light and Dark. rc30 remains the live origin／Worker baseline. Exact rc31
> commit, fingerprint, gates, backup, Worker and rollout evidence remain pending.
>
