...
Restore is protected by a host-wide maintenance admission protocol, not only a Python-thread flag. Every ordinary workflow operation owns one payload-free lease file beside the database. Maintenance obtains an exclusive marker with `O_EXCL`, waits up to 30 seconds for leases from this and other host processes to drain, and rejects new work while the marker exists. A process that loses the marker race never removes the winner's marker. Stale lease files are removed only when the recorded process is no longer alive; the explicit recovery marker is preserved for human review. The same-process condition remains an efficiency aid, while the filesystem marker and leases are the cross-process safety boundary.
