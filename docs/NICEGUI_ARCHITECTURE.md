...
> while rc27 is now the first-level origin rollback beneath the live rc30 origin described above.
...
> **Unified Guest architecture contract:** The live rc30 host enables this path
...
Shell preferences preserve working context. Appearance is an explicit `system | light | dark` value: desktop exposes a labelled menu with the current radio state, while the 320px shell exposes one full-width three-choice radio group. Appearance and interface-sound changes update all desktop／mobile controls in place; enabling sound plays one short success preview and neither action reloads the route. Language controls name the destination by autonym—English／EN from Chinese and 繁體中文／繁中 from English. Language changes still require a translated page reload, so a delegated trusted `input`／`change` guard marks edits inside `<main>` and asks for confirmation before leaving a dirty form. Admin persists through the encrypted user preference adapter, Guest through the bounded session-only adapter, and the Worker through its own browser-local adapter; no layer copies these values across principals. Practice and maintenance banners share one ordered `sy-status-stack`, preventing two valid global states from overlapping. Dialog actions share the `sy-mobile-actions` grammar: desktop keeps a concise action row, while the adaptive shell stacks full-width 48px controls in consequence order.
