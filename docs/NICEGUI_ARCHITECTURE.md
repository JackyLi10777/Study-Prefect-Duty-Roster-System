...
> **Verified production truth (2026-07-30):** Windows runs clean annotated `v1.2.0-rc.39`／`80b9de7ea8abce57b67c6041e580f915a819315e`; canonical Worker `2cb38b05-6091-43be-86d3-d9f3ccae1ceb` serves 100% traffic. The 297-file source fingerprint `df4a2ecb84f242e24349570d209e95405d7251c85810450ce39cf957427b92b9` passed 15／15 gates and the paired rollout passed backup／restore, readiness, staged promotion, canonical health／entrance／viewer and Access fail-closed checks. The first origin rollback is tagged `v1.2.0-rc.35` at commit `570e29f745eef7c1995635d1b187021a8fec6ea4`; the first Worker rollback is `d7069f99-81b4-4388-aa28-383b58bfc68f`. Human acceptance remains open. Older live／candidate wording is historical and superseded by this notice.
>
## Purpose
...
For an off-device copy, use **Settings -> Create handover backup package** after a verified snapshot exists. Immediately before ZIP creation, the workflow copies the selected database and manifest into private staging and re-verifies those exact bytes; the local download contains that exact verified pair plus offline recovery notes. It is generated in memory and is not uploaded or retained as another local copy. Save it only to a school-approved encrypted location. To restore it later, extract both files into `data/backups/` and use the managed restore screen. Do not manually overwrite `data/runtime/sing-yin-roster.sqlite3` while the application is running.
...
Roster generation, publication, manual draft changes, published-duty leave adjustment, pre-generation leave declaration/cancellation, prefect creation/update/archive, bulk import, PDF/report creation, verified snapshot/handover packaging, restore, and host-local support persistence run through one bilingual progress coordinator. Its default is honest phased progress—preparing, safely processing, and complete—with an indeterminate track; the former fixed 14%／56% values are prohibited. Determinate progress is valid only when a service supplies real monotonic `completed／total` data. The durable work runs outside the NiceGUI UI event loop; the existing workflow service remains the sole owner of transactions, fairness changes, and snapshot timing. `_safe_read_action` is reserved for short, bounded, read-only candidate and preview lookups and must not become a write path.
...
Write concurrency is owned below the route layer. Draft generation obtains SQLite `BEGIN IMMEDIATE` before reading the next weekly version, so two clients receive distinct versions instead of colliding after policy work. Prefect edits carry an integer optimistic version and use an atomic compare-and-swap update; a stale editor receives a bilingual reload-and-review state and cannot overwrite the newer record. Managed restore operates on the privately staged exact database／manifest pair, migrates only a supported legacy revision, runs current-head foreign-key and fairness reconciliation, and only then creates the `pre_restore` snapshot and swaps the live file. If post-swap reconnect or audit fails, the workflow installs the safety snapshot automatically; only rollback failure leaves the durable maintenance marker in recovery-review state.
