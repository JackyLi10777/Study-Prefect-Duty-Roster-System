...
if ($AtStartup) {
    ...
    try {
        Register-ScheduledTask -TaskName $TaskName -Action $action -Trigger $trigger -Settings $settings `
            -User $credential.UserName -Password $plainPassword -Description "Starts the local Sing Yin NiceGUI roster host." -Force | Out-Null
    } finally {
        $plainPassword = $null
        $credential = $null
    }
    ...
    Register-ScheduledTask -TaskName $TaskName -Action $action -Trigger $trigger -Settings $settings `
        -Principal $principal -Description "Starts the local Sing Yin NiceGUI roster host." -Force | Out-Null
}
