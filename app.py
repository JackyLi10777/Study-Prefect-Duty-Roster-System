...
"""
    ...
    st.caption(get_text("export_section_caption"))

    col1, col2, col3 = st.columns(3)

    with col1:
        # 明確的 PDF 語言控制：兩個按鈕，分別輸出中文/英文 PDF
        # Use session_state to persist generated bytes so download_button is reliably rendered and stays clickable (fixes non-functional export buttons).
        if st.button(_t("📄 匯出中文 PDF", "📄 Export Chinese PDF"), use_container_width=True):
            ...
            if pdf_bytes:
                st.session_state['pdf_cn_bytes'] = pdf_bytes
                st.session_state['pdf_cn_name'] = f"SYSS_Roster_{datetime.date.today().strftime('%Y%m%d')}_中文.pdf"
                st.session_state['pdf_cn_label'] = _t("💾 下載中文 PDF", "💾 Download Chinese PDF")
        if st.button(_t("📄 Export English PDF", "📄 Export English PDF"), use_container_width=True):
            ...
            pdf_bytes = generate_pdf(st.session_state.roster_df, pdf_report, logo_b64)
            if pdf_bytes:
                st.session_state['pdf_en_bytes'] = pdf_bytes
                st.session_state['pdf_en_name'] = f"SYSS_Roster_{datetime.date.today().strftime('%Y%m%d')}_EN.pdf"
                st.session_state['pdf_en_label'] = _t("💾 下載英文 PDF", "💾 Download English PDF")

        # Persistent download buttons (rendered if bytes present in state). This ensures reliable generation + download trigger for both languages.
        if st.session_state.get('pdf_cn_bytes'):
            st.download_button(
                st.session_state.get('pdf_cn_label', _t("💾 下載中文 PDF", "💾 Download Chinese PDF")),
                st.session_state['pdf_cn_bytes'],
                st.session_state.get('pdf_cn_name', 'roster_cn.pdf'),
                "application/pdf",
                use_container_width=True,
                key="dl_pdf_cn"
            )
        if st.session_state.get('pdf_en_bytes'):
            st.download_button(
                st.session_state.get('pdf_en_label', _t("💾 下載英文 PDF", "💾 Download English PDF")),
                st.session_state['pdf_en_bytes'],
                st.session_state.get('pdf_en_name', 'roster_en.pdf'),
                "application/pdf",
                use_container_width=True,
                key="dl_pdf_en"
            )
