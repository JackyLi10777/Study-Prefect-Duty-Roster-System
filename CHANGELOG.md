...
- Upgraded formal release evidence to schema 3. A candidate must begin clean and finish with the same refreshed runtime fingerprint, file count, commit, tree and clean Git state in `postVerificationSource`; a verifier-induced mutation now fails the release even when every functional gate passed.
- Windows and Worker deployment preflights now reject old／missing post-check evidence. The Worker path also recalculates the current release fingerprint and file count rather than trusting a mutable ignored JSON report.
- The corrected 309-file candidate fingerprint `86e11e061a5b6d0ee08fc7e1dd2b8227c63c0575acd143440f4f41df8da637d3` passed all 15 source gates and the post-verification clean-tree audit. This remains an undeployed rc46 source candidate; the generated production truth remains rc45.
