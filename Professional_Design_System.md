...
Appearance follows the operating-system colour preference by default; if the browser cannot resolve it during server rendering, the safe fallback is dark. The explicit control offers System, Light and Dark as named choices, marks the stored preference, keeps a text alternative and never relies on the icon alone.
...
- Appearance has one explicit stored preference contract: `system`, `light` or `dark`. It never relies on a hidden cycle. Desktop uses one compact trigger whose icon and accessible name describe the **selected preference**, followed by a radio-style menu with all three named choices. Phone uses a full-width three-choice radio group inside Quick settings so every label remains readable at 320 CSS pixels. The selected preference is marked independently of colour; `system` remains distinct from the browser-resolved light／dark result and follows later `prefers-color-scheme` changes. Storage failure may limit persistence but must not prevent the current page from changing safely.
- NiceGUI and the Cloudflare public surface use the same order and terms: System／系統, Light／淺色, Dark／深色. The Worker may retain browser-local storage because it is a public static surface; NiceGUI Admin keeps encrypted user preferences and Guest keeps its bounded session-only preference store. These stores are intentionally separate delivery-boundary adapters, never a reason to change another identity's preference.
