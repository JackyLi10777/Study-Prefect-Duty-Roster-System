...
The recorded production topology is the clean annotated `v1.2.0-rc.43` Windows origin at commit `c8201f33e454d9120c73386642cbf9d737391466` with canonical Worker `394e2205-ae8f-4eef-a13a-e701931e6f0d`. R7 remediation, the selected **B — Operational Rhythm → A — Quiet Editorial Continuity → C — Sacred Service Narrative** composition, semantic motion, honest waiting states, all-route atmosphere registry and Daily Verse v2 surface are included in that deployed source. Rc41／Worker `610092f6-59d4-4fd4-ab3a-3fbf1dd2c64e` are the first paired rollback; rc42 is source-only and was never deployed. `PROJECT_STATUS.md` is the authoritative, versioned deployment record. Runtime health, exact-source verification, controlled deployment and supervised Head Study Prefect／teacher-advisor acceptance are separate claims and must never be collapsed into one status.
...
- The current live topology and immediate rollback pair are recorded only in `PROJECT_STATUS.md`; as of this design-system revision it identifies rc43 as live and rc41 as the first paired origin rollback. Any later runtime source change requires a new immutable candidate, exact-source report, verified backup and isolated restore, matched origin／Worker deployment evidence, canonical online checks, and supervised Head Study Prefect／teacher-advisor acceptance. Historical candidate results never substitute for those gates.
