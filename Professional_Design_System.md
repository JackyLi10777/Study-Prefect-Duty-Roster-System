...
`RosterSchedulePresentation` is the single presentation contract for the editable draft, read-only weekly view, Traditional-Chinese／English PDF, public share, and Guest. Its stable cell states are `assigned`, `vacant`, `room_closed`, and `day_closed`; each state must use text plus structure, not colour alone. `room_closed` means one policy-defined room slot is unavailable. `day_closed` means the complete weekday column is unavailable for that week. Empty editor input is only an unfinished search and must never be styled or persisted as a fifth state.
