...
### 2026.07 product evolution — Luminous Sacred Precision / 明澈而莊重的精準感

`Luminous Sacred Precision` builds on Quiet Precision; it does not replace its operational restraint. The experience should feel illuminated from within by clear purpose, reliable state and a carefully resolved product identity—not by decorative glare. Its signature motif is **Service Weave / 服事經緯**: a restrained, continuous woven-light line that may connect identity, progress and verified completion without becoming a background pattern behind school data.

1. **Material hierarchy remains functional.** Porcelain/mineral-white and cool silver-blue light define the light canvas; obsidian, midnight and deep ink define the dark canvas. Navigation docks, transient sheets, dialogs and the music recovery choice may use a selective luminous edge or glass-like depth. Forms, roster matrices, names, fairness records, warnings, PDFs and consequential confirmations remain opaque.
2. **Light reveals structure.** A low-energy local highlight may clarify a real entry choice, navigation destination or editorial story surface. It never tracks across tables, form fields, names or evidence, and it disappears under reduced motion, forced colours or coarse pointers.
3. **The woven line carries continuity.** It can mark Generate → Review/Export → Published-duty adjustment, identity continuity and verified handover. It may animate once from origin to destination after an accepted state change; it never loops, chases the pointer or implies progress that has not occurred.
4. **Sacred warmth stays distinct.** Warm gold and parchment remain reserved for Scripture, reflection and truly consequential stewardship moments. Teal/cyan identify action and verified stability; they do not wash the whole product in one accent.
5. **Every flourish has a removal test.** If removing a glow, line, image or transition does not reduce orientation, feedback, comprehension or brand recognition, it is decorative and should remain removed.
6. **Public, Guest, Admin and Viewer share one identity but not one density.** The public entrance explains value and offers safe identity choices; Guest and Admin enter the same operational routes; Viewer stays silent, print-first and read-only. Visual parity must never weaken server-enforced capability boundaries.

The executable token contract is versioned with this section. `luminous-edge`, `woven-line`, `material-transient` and `material-operational` are semantic roles, not effects that pages may reinterpret independently.
...
Appearance follows the operating-system colour preference only while no explicit preference has been stored; if the browser cannot resolve it during server rendering, the safe fallback is light. `system` is therefore an initialization state, not a visible choice. The single visible control reports the currently resolved Light or Dark mode and names the opposite action. Its first activation stores the opposite of the resolved mode; later activations alternate explicit Light and Dark. It keeps a text alternative and never relies on the icon alone. Public／Viewer never reads or continuously synchronizes Admin／Guest preferences. Only a deliberate transition into Admin／Guest may stage an explicit normalized `light`／`dark` hint for at most 120 seconds. The Worker validates it, carries it inside the signed session and request-bound principal, and clears the staging cookie when the session is minted; the destination adopts it only while unset, and an existing preference always wins.
...
### Selected composition order — B → A → C

The three reviewed directions are not three themes to mix equally. They form one ownership hierarchy:

1. **B — Operational Rhythm is the product skeleton.** Weekly roster routes must expose the current stage, the owning hub, a deterministic return path and only the destinations that are actually available. The shared `WorkflowStep`／route-trail／back-action components own this sequence. The narrow woven position line reports where the operator is; it must not imply completion, elapsed time or a successful write.
2. **A — Quiet Editorial Continuity is the visual and reading layer.** Shell geometry, typography, spacing, material surfaces and page-to-page continuity remain calm and consistent around the operational skeleton. A may refine hierarchy but may not move, rename or obscure a required B action.
3. **C — Sacred Service Narrative is reserved for Scripture.** The Daily Verse reading sequence may use the sacred indigo／parchment language and contemplative pacing. Its visual grammar does not migrate into forms, tables, fairness evidence, status banners or PDF output.

This order is a testable composition contract, not an inspiration list. When a proposal conflicts with the hierarchy, B wins for operational routes, A wins for ordinary reading continuity, and C wins only inside the explicitly sacred surface. Mobile, tablet and desktop may rearrange the same semantic order, but they may not create a second workflow or omit its return path.
...
The exact production origin, Worker, migration, rollback and acceptance state is owned only by `docs/status/current-release.json` and its generated `docs/status/CURRENT_STATUS.md` view. This living design standard must not copy mutable release identifiers: doing so previously left its narrative several releases behind the canonical status. Runtime health, exact-source verification, controlled deployment and supervised Head Study Prefect／teacher-advisor acceptance are separate claims and must never be collapsed into one status.
...
Music is page-context-aware and makes one controlled autoplay attempt at the 50% browser-local default after a page becomes ready. The operator can pause it or disable browser-local autoplay from the current page or Settings; that preference persists until changed. Browser media policy remains authoritative, so a blocked attempt becomes a clear explicit-recovery state rather than repeated or hidden retries. The implemented system has one `Follow appearance` preference plus two independently selectable profiles:
...
`RosterSchedulePresentation` is the single presentation contract for the editable draft, read-only weekly view, Traditional-Chinese／English PDF, public share, and Guest. Its stable cell states are `assigned`, `vacant`, `room_closed`, `unavailable`, and `day_closed`; each state must use text plus structure, not colour alone. `room_closed` means one policy-defined room slot is closed. `unavailable` means an operator explicitly closed one cell for this week. `day_closed` means the complete weekday column is unavailable for that week. Their precedence is `day_closed > room_closed > unavailable > assigned/vacant`. Empty editor input is only an unfinished search and must never be styled or persisted as another state.

The editable matrix uses one interaction model across pointer, touch and keyboard. Pointer movement becomes a drag only after an 8 CSS-pixel threshold; touch uses source-then-target selection; keyboard uses roving focus, arrows, `Enter`/`F2`, `Space`, `Escape`, undo and redo. Movement and swaps are staged locally, announced, and committed once through the versioned draft transaction. Animation may clarify the state change but may not move, tilt or resize the button or cell container.

The desktop grid may behave like a spreadsheet, but it is not a collection of independent write buttons. Click, `Enter`, or `F2` opens one shared cell editor; a Chinese-character query narrows only policy-valid candidates, while `X`, `×`, `空缺`, and `待安排` reveal an explicit vacancy choice. `Escape` cancels the current edit, `Ctrl`／`Cmd` + `Z` reverses the latest local edit, and a same-day selection may stage an atomic two-cell swap. All edits remain visibly pending until one **Review and save** action validates and writes the complete patch. Pending, conflicting, vacant, room-closed, and day-closed cells require distinct copy, icon／border treatment, focus, and forced-colour equivalents.

Each weekday header owns an accessible **Closed all day** toggle. Before save it is reversible; when existing assignments would be removed, confirmation states the exact count. A saved reopen produces vacancies rather than restoring stale assignments. Published weeks never expose this editor or toggle: the operator must withdraw the published week before rebuilding it. Conflict UI preserves local input and presents **Reload／Compare changes／Reapply** rather than silently accepting last-writer-wins.

At tablet widths the component chooses the matrix only when every interactive cell retains a 44px target and readable label. Otherwise it uses the day-card presentation used on phones. Both presentations consume the same model and submit the same batch patch; responsive design must not create a second policy or persistence path. The PDF keeps the same row order, weekday dates, English duty-post names, service times, Chinese names, vacancies, room closures, week-local unavailable cells (`本週不開放 / Unavailable`), and whole-day closures, while omitting internal closure reasons. It applies `day_closed > room_closed > unavailable > assigned/vacant`; an unavailable cell must never be rendered as a vacancy or policy-defined room closure.
...
- The mobile drawer has one authoritative requested-open state while Quasar is animating. The fixed top-right close control always shows and means **Close**; the external More／menu trigger alone may morph between menu and close. `aria-expanded`, inert background, focus trap, backdrop and rendered visibility must settle from the same state, including rapid taps, breakpoint changes, reconnect and route cleanup. A hidden desktop trigger is never used as the mobile state owner.
...
- Language controls always name the **destination language in its own language**. A Chinese interface offers `English` (`EN` only in the compact desktop header); an English interface offers `繁體中文` (`繁中` only in the compact desktop header). Visible text, tooltip／accessible action name, stored locale and `<html lang>` must describe the same destination. The public entrance and encrypted viewer remain intentionally bilingual and therefore do not add a redundant locale preference.
- **The rc30 three-choice appearance selector is historical and superseded.** Every primary interface now exposes one binary Light／Dark button: explicit Light changes to explicit Dark; explicit Dark changes to explicit Light. `system` remains only the missing／unset initialization value. While unset, the resolved appearance follows `prefers-color-scheme`; the first activation stores the opposite of that resolved appearance and no later activation can enter a hidden third state. An operating-system change updates the resolved appearance only while the preference remains unset.
- The appearance icon describes the **current resolved result**—sun in Light, moon in Dark—while its tooltip and accessible name describe the **next action**. `aria-pressed="true"` represents resolved Dark and `false` represents resolved Light. State, icon, accessible name and stored explicit preference must remain synchronized after activation, reload, navigation, reconnect and cross-tab notification. The initial resolved frame does not animate; later changes use the state transition below.
- NiceGUI and the Cloudflare public surface share this binary behavior even though their storage adapters remain deliberately different. The Worker may retain an explicit browser-local Light／Dark value; NiceGUI Admin keeps encrypted user preferences and Guest keeps its bounded session-only preference store. Storage failure may limit persistence but must not prevent the current page from changing safely. No delivery boundary may reintroduce a menu, radio group, select element or `System` click state.
- Public support is part of the same entrance family, not a utility exception. Its theme control reuses the entrance sun／moon anatomy, current-state glyph, next-action accessible name, 180ms bounded transition and reduced-motion fallback. Returning from `/support` must preserve the resolved appearance without exposing a visually inferior text-only theme control.

#### Shared framed header controls

Language, sound, appearance and logout are one component family, not four page-local CSS treatments. Their common anatomy is a fixed 44px minimum target, 12px radius, visible one-pixel semantic edge, solid theme surface, shared inset highlight, restrained control shadow, centered 20–22px icon and stable internal spacing. Language alone may be wider for `EN`／`繁中`; logout alone may gain a muted danger tint on hover, focus and activation. All members look related at rest before hover.

The token layer owns the component grammar: `--sy-control-edge` owns the border, `--sy-surface`／`--sy-material-transient` own the rest surface, `--sy-control-shadow` and `--sy-control-shadow-hover` own depth, `12px` is the assigned compact radius, `--sy-focus` owns the focus ring, and `--sy-role-action-soft`／`--sy-role-danger-soft` own semantic state surfaces. Page CSS must not recreate these borders, colours, shadows, radii or timings. The shared component owner supplies rest, hover, focus-visible, active, pressed／selected, disabled and busy states in both themes and forced colours.

Language always names the destination locale; sound and appearance expose synchronized `aria-pressed`; logout retains a distinct action label and cannot look like a persistent toggle. Press feedback is contained within the fixed box. Theme and sound glyphs may crossfade, scale and rotate by at most a small bounded amount during the 180ms state transition, but the host never moves, tilts or changes size. `prefers-reduced-motion: reduce` removes the glyph motion while retaining the immediate icon, label, border and state change. Animation is never required for persistence, session revocation or navigation.
...
- Draft and directory editing state belongs to typed page-session models (`DraftEditSession` and `PrefectEditSession`), not nested route dictionaries. They own the reviewed baseline, pending values, dirty count, stable command ID, local undo／redo where applicable and conflict reapply. Route code owns widgets and copy; the workflow remains the final policy and persistence authority.
- A multi-row prefect save is one operator intent and therefore one transaction. Every targeted row is validated against its expected version before any mutation; one invalid or stale row means zero rows are saved. The interface must say this explicitly, retain all local edits, identify the conflicting fields and reuse the same command ID only for a retry of that exact intent.
...
- A period-report preview is a read action, so its progress language says “checking” or “building”, never “saving”. Download actions use the already reviewed immutable model and do not imply that JSON created a backup or that scheduled allocation became verified service. Admin and Guest downloads share one bounded generated-file registry, but every ticket is bound to its issuing access mode and live session; cross-mode or cross-session replay is rejected, Guest cannot consume the reserved Admin capacity, and the credentialed browser fetch must verify the HTTP status and exact MIME type before saving. A failed delivery names a safe retry or recovery action and an `OP-...` support reference instead of delegating the failure to the browser download shelf.
...
- The prefect directory may expose allowlisted low-risk fields inline on both desktop rows and mobile cards, but Chinese identity, role, fairness history and archive state remain confirmation-dialog operations. Filtering and stable sorting apply to the merged reviewed-plus-pending model, so changing a filter never discards an unsaved row.
...
- Recovery: state what will be restored, what safety snapshot is created first, and that the selection must be verified. A pending backup obligation is distinct from an unproven restore marker: the former may be repaired safely before writes reopen, while the latter starts the application in diagnostic-only mode before migrations, database sessions or journal mutation. Neither state may expose an apparently enabled business-write action.
...
- The bilingual Guest PDF is A4 landscape, keeps all names Chinese, carries an unmistakable `DEMO` identity and is generated in memory behind a one-shot `no-store` download. Its ticket is valid only for the issuing Guest access mode and session; it cannot be replayed as Admin or by another Guest session, and Guest load cannot consume the generated-file registry's reserved Admin capacity. Retention begins only if the visitor explicitly saves it.
...
- Candidate lists and final save validation use the same availability, leave, role, same-day uniqueness, and no-consecutive-duty constraints. Ordinary-room generation uses fairness-ordered deterministic backtracking with a daily bipartite-matching forward check and failed-state cache; it must either return a complete valid roster or a controlled no-complete-roster result, never a partial allocation. Final validation independently rejects empty assignments, wrong duty weights, unknown people, and every missing or extra day／post slot. A recommendation is never treated as sufficient proof at save time.
...
- Prefect writes, pre-generation leave changes, draft generation and the final draft-patch save begin with SQLite `BEGIN IMMEDIATE`. Prefect update／archive compare the reviewed record version; the grid compares the roster-week version and validates the complete staged matrix, so stale input is preserved for reload／comparison／reapplication rather than overwriting newer state.
- Restore uses a host-wide exclusive maintenance marker plus payload-free operation leases. It waits for active operations in every application process to drain, rejects new writes, privately stages and re-verifies the exact database／manifest byte pair, preflights the complete SQLAlchemy／Alembic schema and fairness state on a clone, and preserves a recovery marker only when automatic rollback cannot be proven safe. If that marker exists at startup, the application enters diagnostic-only mode before migrations, database-session creation or journal mutation; every business write, including school-year rollover and sharing, stays rejected until recovery is proven. Adjacent SQLite sidecars, malformed／non-object manifests, pending backup obligations and unknown or future migration revisions are rejected; a supported legacy chain from `0007` may migrate only in isolation before current-schema validation.
...
- The current live topology and immediate compatible rollback procedure are read from `docs/status/CURRENT_STATUS.md`, generated from the machine-readable status owner. Any runtime source change requires a new immutable candidate, exact-source report, verified backup and isolated restore, matched origin／Worker deployment evidence, canonical online checks, and supervised Head Study Prefect／teacher-advisor acceptance. Historical candidate results never substitute for those gates, and this design standard must not embed another mutable copy of the current identity.
...
- Handover readiness uses a compact three-column desktop grid and one-column phone sequence; never render each small status as an oversized full-width card. Machine evidence and human acceptance are visually paired but semantically separate. Pass, running, stale, failed, missing, and unreadable states require icon, heading, explanatory copy, and a safe next action rather than colour alone. Human work is grouped by `nicegui_app/ui/acceptance_readiness.py` into a small number of supervised sessions, while `docs/ACCEPTANCE_EVIDENCE.md` remains the detailed authority. Counts are derived from that catalog, tests require exact H／A identifier parity with the matrix, and the downloadable worksheet begins unchecked; the interface never self-signs or turns a machine pass into human approval.
