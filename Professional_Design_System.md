...
The recorded production topology is the clean annotated `v1.2.0-rc.35` Windows origin at commit `570e29f745eef7c1995635d1b187021a8fec6ea4` with canonical Worker `d7069f99-81b4-4388-aa28-383b58bfc68f`. The integrated R7 remediation and B → A → C composition remain a later source candidate until a new clean tag completes exact-source verification and controlled rollout. `PROJECT_STATUS.md` is the authoritative, versioned deployment record. Runtime health, exact-source verification, controlled deployment and supervised Head Study Prefect／teacher-advisor acceptance are separate claims and must never be collapsed into one status.
