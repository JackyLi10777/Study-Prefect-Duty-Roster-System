...
5. **Motion has three speeds:** 90 ms press acknowledgement, 180 ms state change, and 260 ms layer/entry transition. Exit is faster than entry; motion uses transform/opacity and remains interruptible. “Calm” does not mean imperceptible: semantic icons may visibly rotate 22–90°, travel 5–6 px, change scale and resolve a geometric halo when the operator points, focuses or presses. The shape change must be obvious enough to confirm affordance, run only in response to interaction, and become completely static under reduced motion.
...
- Music is a separate comfort layer whose local player may make one visible page-ready attempt at a browser-local default of 35%, and always offers immediate pause/off controls. The 35% level is a starting point rather than forced loudness: an exact legacy 24% default is upgraded once, while every other operator-selected level is preserved. When two routes resolve to the same local track, the next route restores that track's session position and playing／paused state instead of restarting it; this continuity uses bounded `sessionStorage`, never the roster database or permanent browser storage. A YouTube player must remain fully visible with native controls, never autoplay, and never sit behind a form, name, table, warning, roster, fairness record, or PDF. Public playlist playback does not require sign-in; optional API search and saved playlist names must never carry student data.
