...
from roster_core.policy_settings import (
    PolicyCommandConflict, PolicyStorageError, PolicyVersionConflict, StoredPolicyRevision,
)
    ...
    def read(self, year_start: int, revision: int | None = None) -> StoredPolicyRevision | None:
        ...
        try:
            with self._engine.connect() as connection:
                ...
                stored = StoredPolicyRevision(row["year_start"], row["revision"], row["document"]) if row else None
            return stored
        except SQLAlchemyError as error:
        ...
        try:
            with self._engine.connect() as connection:
                ...
                if receipt is not None:
                    ...
                    row = connection.execute(select(_revisions).where(
                        _revisions.c.year_start == receipt["year_start"],
                        _revisions.c.revision == receipt["revision"],
                    )).mappings().one()
                    stored = StoredPolicyRevision(row["year_start"], row["revision"], row["document"])
                    connection.rollback()
                    return stored
                ...
                connection.execute(insert(_commands).values(
                    command_id=command_id, request_digest=request_digest, year_start=year_start, revision=new_revision,
                ))
                connection.commit()
