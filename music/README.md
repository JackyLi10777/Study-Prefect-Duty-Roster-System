...
- The public login entrance and authenticated workspace each make one honest playback attempt at a browser-local default of **50%**. Every explicit operator-selected level remains authoritative; only exact legacy workbench defaults of 24%／35% are migrated once. With no explicit entry-sound choice, the Administrator／Guest CTA is the trusted default-music retry: playback success, failure, or the bounded startup timeout continues exactly once to the selected identity. **Default: Enter with music** and **Continue quietly** are optional preference／recovery actions, not a gate. Generic page interactions never trigger a hidden retry.
