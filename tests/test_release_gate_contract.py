...
def test_windows_powershell_bridge_uses_same_validator_without_deploying(tmp_path):
...
foreach ($case in $cases) {{
    ...
    try {{
        ...
        $passed = ($gate.requiredChecks.Count -eq {len(gates.REQUIRED_CHECK_IDENTITIES)}) -and
                  ($gate.report.sourceCommit -ceq {quote('a' * 40)})
        if ($passed) {{
            # A later atomic report replacement must not replace the snapshot
            # returned by the validator and consumed by either deploy caller.
    ...
    if ($passed -ne $case.valid) {{ throw 'Consumer mismatch' }}
}}
